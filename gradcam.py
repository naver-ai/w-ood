...
class _BaseWrapper(object):
    ...
    def forward(self, image, get_prob=False, get_feature=False):
        self.image_shape = image.shape[2:]
        if get_feature:
            self.logits, out_features = self.model(image, separate=True, get_feature=get_feature)
        else:
        ...
        if get_prob:
            if get_feature:
                return F.softmax(self.logits_vec, dim=1), out_features
            else:
                return F.softmax(self.logits_vec, dim=1)

        else:
            if get_feature:
                return self.logits, out_features  # ordered results
            else:
                return self.logits
