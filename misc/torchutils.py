...
def gndp2d(x, valid_cat, keepdims=False, score_th=0.5, is_meta=False, gndp_nega=0):
    # print("gndp")
    # print(x.shape)
    # print(valid_cat)
    ...
    for b in range(x.shape[0]):
        ...
        # print(valid_cat_here)
        for ccc in range(20):
            if ccc in valid_cat_here:
                # print(ccc, regions[ccc].max())
                norm_x[b, ccc] = (x[b, ccc] / x[b, ccc].max()).detach()
            else:
                norm_x[b, ccc, :] = -1

    # mask_x[norm_x>0.5]=0
    out = torch.mean((x*(norm_x<score_th)).view(x.size(0), x.size(1), -1), -1) - torch.mean((x*(norm_x>=0.7)).view(x.size(0), x.size(1), -1), -1) * gndp_nega
    # print("out", out, '\n', torch.mean(x.view(x.size(0), x.size(1), -1), -1), out.shape, torch.mean(x.view(x.size(0), x.size(1), -1), -1).shape)
